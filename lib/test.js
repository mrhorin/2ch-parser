...
__webpack_require__(5)(_superagent2.default);
...
module.exports = require("superagent-charset");
...
module.exports = function () {
  ...
  _createClass(Board, [{
    ...
    get: function get() {
      ...
      return new Promise(function (resolve, reject) {
        var charCode = _url_parser2.default.isShitaraba(_this.url) ? 'EUC-JP' : 'Shift_JIS';
        _superagent2.default.get(_this.subjectUrl).charset(charCode).timeout(5000).end(function (err, res) {
          if (err) {
            reject(err);
...
var hasProperty = __webpack_require__(14);
...
describe('Board', function () {
  ...
  describe('#fetchThreads', function () {
    it('StatusCode is 200 in case of jbbs board URL.', function (done) {
      var jbbs = new _board2.default(url.board.jbbs);
      jbbs.fetchThreads(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
    });

    it('StatusCode is 200 in case of 2ch.sc board URL.', function (done) {
      var sc = new _board2.default(url.board.sc);
      sc.fetchThreads(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
    });

    it('StatusCode is 200 in case of open board URL.', function (done) {
      var open = new _board2.default(url.board.open);
      open.fetchThreads(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
    });

    it('Error code is ENOTFOUND in case of invalid URL.', function (done) {
      var invalid = new _board2.default(url.board.invalid);
      invalid.fetchThreads(function (res) {
        // The reson is that superagent don't resolve domain name.
        _chai.assert.equal(res.code, 'ENOTFOUND');
        done();
      });
    });
  });

  describe('#threadsPromise', function () {
    it('Make it possible to catch ENOTFOUND on catch method when it receives invalid URL.', function (done) {
      var invalid = new _board2.default(url.board.invalid);
      invalid.threadsPromise.catch(function (err) {
        _chai.assert.equal(err.code, 'ENOTFOUND');
        done();
      });
    });
  });
...
describe('Thread', function () {
  ...
  describe('#fetchNewPosts', function () {
    it('StatusCode is 200 in case of shitaraba thread URL.', function (done) {
      var jbbs = new _thread2.default(url.thread.jbbs);
      jbbs.fetchNewPosts(function (res) {
        _chai.assert.isTrue(res.statusCode == 200, 'statuCode is 200');
        done();
      });
    });

    it('StatusCode is 206 or 304 in case of 2ch.sc thread URL.', function (done) {
      var sc = new _thread2.default(url.thread.sc);
      sc.fetchNewPosts(function (res) {
        _chai.assert.isTrue(res.statusCode == 206 || res.statusCode == 304, 'statuCode is 206 or 304');
        done();
      });
    });
  ...
  describe('#fetchAllPosts', function () {
    it('StatusCode is 200 in case of shitaraba thread URL.', function (done) {
      var jbbs = new _thread2.default(url.thread.jbbs);
      jbbs.fetchAllPosts(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
    });

    it('StatusCode is 200 in case of open thread URL.', function (done) {
      var jbbs = new _thread2.default(url.thread.open);
      jbbs.fetchAllPosts(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
    ...
    it('StatusCode is 200 in case of 2ch.sc thread URL.', function (done) {
      var jbbs = new _thread2.default(url.thread.sc);
      jbbs.fetchAllPosts(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
  ...
  describe('#newPostsPromise', function () {
    it('Status code is 200 in resolve block in case of shitaraba thread URL.', function (done) {
      var jbbs = new _thread2.default(url.thread.jbbs);
      jbbs.newPostsPromise.then(function (res) {
        _chai.assert.equal(res.statusCode, 200);
        done();
      });
    });
    ...
    it('Error code is ENOTFOUND in case of invalid URL.', function (done) {
      var invalid = new _thread2.default(url.thread.invalid);
      invalid.newPostsPromise.catch(function (err) {
        // The reson is that superagent don't  resolve domain name
        _chai.assert.equal(err.code, 'ENOTFOUND');
        done();
      });
    ...
    it('Make it possible to catch error of 404 in case of invalid URL on shitaraba.', function (done) {
      var jbbs = new _thread2.default('http://jbbs.shitaraba.net/return/404/hoge');
      jbbs.newPostsPromise.catch(function (err) {
        // The reson is that it happen timeout in superagent.
        _chai.assert.isTrue(err.status == 404, 'in catch');
        done();
      });
    });
  });
...
describe('UrlParser', function () {
  ...
  describe('#isShitaraba', function () {
    it(url.board.jbbs + ' is true.', function () {
      _chai.assert.isOk(_url_parser2.default.isShitaraba(url.board.jbbs), url.board.jbbs + ' is ok.');
    });

    it(url.thread.jbbs + ' is true.', function () {
      _chai.assert.isOk(_url_parser2.default.isShitaraba(url.thread.jbbs), url.thread.jbbs + ' is ok.');
    });

    it(url.board.sc + ' is false.', function () {
      _chai.assert.isNotOk(_url_parser2.default.isShitaraba(url.board.sc), url.board.sc + ' will fail.');
    });
  });

  describe('#getBoardUrl', function () {
    it('In case of ' + url.thread.jbbs + ' returns ' + url.board.jbbs + '.', function () {
      _chai.assert.equal(_url_parser2.default.getBoardUrl(url.thread.jbbs), url.board.jbbs);
    });
  });

  describe('#getThreadUrl', function () {
    it('In case of ' + url.board.jbbs + ' returns ' + url.thread.jbbs, function () {
      _chai.assert.equal(_url_parser2.default.getThreadUrl(url.board.jbbs, '1108684328'), url.thread.jbbs);
    });

    it('In case of ' + url.board.open + '/ returns ' + url.thread.open, function () {
      _chai.assert.equal(_url_parser2.default.getThreadUrl(url.board.open + '/', '1365706412'), url.thread.open);
    });

    it('In case of ' + url.board.sc + '/l50 returns ' + url.thread.sc, function () {
      _chai.assert.equal(_url_parser2.default.getThreadUrl(url.board.sc + '/l50', '1397594191'), url.thread.sc);
    });
  });
});
...
var getPathInfo = __webpack_require__(13);
...
exports.getPathInfo = __webpack_require__(13);

/*!
 * Check if a property exists
 */

exports.hasProperty = __webpack_require__(14);
